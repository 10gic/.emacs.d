...
;; Note : il est important d'identifier des caract猫res en fin de ligne
...
;; todo : parsing plus compliqu茅: {$obj->values[3]->name}
...
;; todo : passer de r猫gle en r猫gle et mettre un \n 脿 la fin
...
(defun web-mode-previous-line (pos limit)
  (save-excursion
    (let (beg end line (continue t))
      ...
      (if (<= (point) limit)
          ;;todo : affiner (le + 3 n est pas g茅n茅rique cf. <?php <% <%- etc.)
     ...
     (controls
      ...
      (while continue
        (cond
         ...
         (t
          ;; TODO : est il n茅cessaire de faire un reverse sur controls si on doit matcher backward
...
(defun web-mode-dom-apostrophes-replace ()
  "Replace char(') with char(鈥? in the html contents of the buffer."
  (interactive)
  (save-excursion
...
(defun web-mode-dom-entities-replace ()
  "Replace html entities (e.g. &eacute; &#233; or &#x00E9; become 茅)"
  (interactive)
  (save-excursion
...
(defun web-mode-trace (msg)
  (let (sub)
    ...
    (message "%18s: time elapsed = %Ss %9S碌s" msg (nth 1 sub) (nth 2 sub))
    ))
